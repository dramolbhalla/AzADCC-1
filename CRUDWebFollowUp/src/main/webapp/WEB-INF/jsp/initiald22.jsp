...
function validate()
{ 
	var arths = document.getElementsByName('arth');
	var arth;
	for(var i = 0; i < arths.length; i++){
	    if(arths[i].checked){
	    	arth = arths[i].value;
	    }
	}
	var artypes = document.getElementsByName('artype');
	var artype;
	for(var i = 0; i < artypes.length; i++){
	    if(artypes[i].checked){
	    	artype = artypes[i].value;
	    }
	}
	
	var artypex = document.getElementById("artypex").value;
    if ((arth == 1 && artype == 3) && artypex.match(""))
    {
    	window.alert( "Please enter the aethritis type!" );
        return false;
    }
    
    var sleepoths = document.getElementsByName('sleepoth');
    var sleeptx = document.getElementById("sleeptx").value;
	var sleepoth;
	for(var i = 0; i < sleepoths.length; i++){
	    if(sleepoths[i].checked){
	    	sleepoth = sleepoths[i].value;
	    }
	}
	if (sleepoth == 1 && sleeptx.match(""))
    {
    	window.alert( "Please enter the Sleep disorder" );
        return false;
    }
	
	var antiencxs = document.getElementsByName('antienc');
	var antiencx = document.getElementById("antiencx").value;
	var antienc;
	for(var i = 0; i < antiencxs.length; i++){
	    if(antiencxs[i].checked){
	    	antienc = antiencxs[i].value;
	    }
	}
	if (antienc == 1 && antiencx == '')
    {
    	window.alert( "Please enter the Antibody" );
        return false;
    }
	
	var othconds = document.getElementsByName('othcond');
	var othcondx = document.getElementById("othcondx").value;
	var othcond;
	for(var i = 0; i < othconds.length; i++){
	    if(othconds[i].checked){
	    	othcond = othconds[i].value;
	    }
	}
	
	if (othcond == 1 && othcondx == '')
    {
    	window.alert( "Please enter other medical conditions" );
        return false;
    }
}
</script>
...
<form:form action="initiald22.do" method="POST" commandName="initiald22" onsubmit="return validate();">
 ...
 <tr >
  ...
  <tr>
   ...
   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; 11a.  If yes, what type?<br>
	...
	<form:input type="radio" name="artype" required ="required" path="artype" value="2" style="margin-left:90px;"/>3 Other (SPECIFY) : <form:input type="text" name="artypex" path="artypex" style="margin-left:0px; width:300px;"/><br />
